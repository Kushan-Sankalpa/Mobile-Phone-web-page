...
const slides = [
  {
    id: 1,
    video: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/googlepixel-zLKE2ISzdrASuKOc08RhnFoNYo93M6.mp4",
    poster: "/google-pixel-phone.jpg",
    title: "Google Pixel",
  },
  {
    id: 2,
    video: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/AppleShort-ioQTH3tg3LOr0klY8l1SNLfeuLffUg.mp4",
...
export default function VideoSlider() {
  ...
  const nextSlide = () => { setCurrentSlide((s) => (s + 1) % slides.length); setProgress(0); };
  const prevSlide = () => { setCurrentSlide((s) => (s - 1 + slides.length) % slides.length); setProgress(0); };
  ...
  return (
    <div className="relative w-full h-screen bg-black overflow-hidden pt-16" role="region" aria-label="Video slider">
      ...
      <div className="absolute bottom-0 left-0 right-0 h-1 bg-white/20" role="progressbar" aria-valuemin={0} aria-valuemax={100}>
        <div className="h-full bg-white transition-all duration-100" style={{ width: `${Math.round(progress)}%` }} />
      </div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 flex gap-2" role="tablist">
        {slides.map((_, index) => (
          <button
            ...
            className={`w-2 h-2 rounded-full transition-all focus:outline-none focus:ring-2 focus:ring-white ${index === currentSlide ? "bg-white w-8" : "bg-white/50 hover:bg-white/75"}`}
