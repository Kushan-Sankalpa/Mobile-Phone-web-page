...
import { fetchSpeakers } from "@/lib/catalog";
...
export default function SpeakersList({ brand = "" }) {
  ...
  // title changes based on brand
  const title = useMemo(() => {
    if (!brand) return "All Speakers";
    return `${brand} Speakers`;
  }, [brand]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const all = await fetchSpeakers();

        if (!alive) return;

        // If brand is provided, filter locally (case-insensitive)
        ...
        if (!alive) return;
        setErr(e?.message || "Failed to load speakers");
      } finally {
  ...
  return (
    <section className={styles.wrap} aria-label="Speakers catalog">
